...
import { getProjectStatus, getProjectCategories } from '../utils/projectUtils';
...
export const ProjectFilters = memo<ProjectFiltersProps>(({ 
  ...
  const presentStatuses = new Set((projects ?? []).map(getProjectStatus));
  const presentCategories = new Set((projects ?? []).flatMap(getProjectCategories));
  ...
  ].filter(({ key }) => key === 'all' || !projects || presentStatuses.has(key));
